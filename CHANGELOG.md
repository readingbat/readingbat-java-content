...
`1.0.0` is the first tagged release; everything below it predates tagging and is
...
repository; the sections below record the history that led here.

### Changed
- Bumped the Gradle JVM toolchain to **25**, and matched CI's JDK to it so lint and
  tests run on the same Java version the build targets (#16).
- Upgraded the Gradle wrapper `9.5.1` → `9.6.1`.
- Upgraded dependencies: readingbat-core `3.1.8` → `3.3.1`, common-utils `2.9.0` → `3.2.2`,
  Kotest `6.1.11` → `6.2.3`, Kotlin `2.4.0` → `2.4.10`, Ktor `3.5.0` → `3.5.1`,
  kotlinter `5.5.0` → `5.6.0`, detekt `2.0.0-alpha.3` → `2.0.0-alpha.5`, and the
  versions plugin `0.54.0` → `0.57.0`.
- Moved the versions plugin id from `com.github.ben-manes.versions` to
  `io.github.ben-manes.versions` (#17).
- Refactored `build.gradle.kts` into `configure*` extension functions, switched the
  fat jar to Ktor's `fatJar` block, and added the unused-return-value checker to
  production compilation (#16).
- Trimmed `CLAUDE.md` down to guidance that cannot be derived from the repo itself (#17).

### Fixed
- Updated the correct-answers test for the readingbat-kotest `3.2.1` API, where
  `Challenge.correctAnswers` became a suspend function rather than a property (#16).

### Removed
- Dropped the obsolete Heroku `heroku` and `logs` Makefile targets (#16).
